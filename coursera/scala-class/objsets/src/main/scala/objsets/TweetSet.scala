...
abstract class TweetSet {
  ...
  /**
   * This is a helper method for 'mostReweeted'
   */
  ...
  def descendingByRetweetAcc(acc: TweetList): TweetList = { 
    ...
    if(acc.isEmpty)
      if(mr == Nil)
        ...
        remove(mr).descendingByRetweetAcc(new Cons(mr, acc))
    else
      if(mr== Nil)
        acc
      else
        remove(mr).descendingByRetweetAcc(new Cons(acc.head, new Cons(mr, acc.tail)))
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet = 
    if(elem.retweets > acc.retweets) 
      ...
      left.union(right).mostRetweetedAcc(acc)
