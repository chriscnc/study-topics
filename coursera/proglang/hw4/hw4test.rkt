...
(define tests
  (test-suite
   ...
   (check-equal? (sequence 3 11 2) (list 3 5 7 9 11) "Inclusive High")
   (check-equal? (sequence 3 8 3) (list 3 6) "Exclusive High")
   (check-equal? (sequence 3 2 1) '() "Invalid Low/High")
   ...
   (check-equal? (stream-for-n-steps (lambda () (cons 1 ones)) 0) '() "steam-for-n-steps empty list")
   (check-equal? (stream-for-n-steps (lambda () (cons 1 ones)) 4) (list 1 1 1 1) "steam-for-n-steps 4 steps")
   ...
   (check-equal? (stream-for-n-steps dan-then-dog 3) (list "dan.jpg" "dog.jpg" "dan.jpg") "dan-then-dog n > 1 test")
   
   ; stream-add-zero test
   (check-equal? (stream-for-n-steps (stream-add-zero ones) 1) (list (cons 0 1)) "stream-add-zero test")
   (check-equal? (stream-for-n-steps (stream-add-zero nats) 3) (list (cons 0 1) (cons 0 2) (cons 0 3)) "stream-add-zero n>0 test")
